...
SELECT distinct '@siteAbbrev' as SITE_ABBREV,
   ...
   CAST( (current_date - interval '@dataLatencyNumDays day') as date) as UPDATE_DATE, --change integer based on your site's data latency
   CAST( (current_date + interval '@daysBetweenSubmissions day') as date) as NEXT_SUBMISSION_DATE
...
SELECT 
      'DEMOGRAPHIC' TABLE_NAME,
         COUNT(*) as DUP_COUNT
			...
			GROUP BY DEMOGRAPHIC.PATID 
         HAVING COUNT(*) > 1
UNION
SELECT 'ENCOUNTER'  TABLE_NAME,
         COUNT(*) as DUP_COUNT
...
SELECT 'DISPENSING' TABLE_NAME,
	     COUNT(*) as DUP_COUNT
			...
			GROUP BY DISPENSINGID 
         HAVING COUNT(*) > 1
UNION
SELECT 'IMMUNIZATION'  TABLE_NAME,
	     COUNT(*) as DUP_COUNT
			...
			GROUP BY IMMUNIZATIONID 
         HAVING COUNT(*) > 1
UNION
SELECT 'LAB_RESULT_CM' TABLE_NAME,
         COUNT(*) as DUP_COUNT
			...
			HAVING COUNT(*) > 1
UNION 
SELECT 'LDS_ADDRESS_HISTORY' TABLE_NAME,
         COUNT(*) as DUP_COUNT
...
SELECT 'DEMOGRAPHIC' TABLE_NAME,
   count(*) as ROW_COUNT
    FROM @cdmDatabaseSchema.DEMOGRAPHIC 
    JOIN @resultsDatabaseSchema.N3C_COHORT ON DEMOGRAPHIC.PATID = N3C_COHORT.PATID
UNION 
SELECT 'ENCOUNTER'  TABLE_NAME,
   count(*)  as ROW_COUNT
   FROM @cdmDatabaseSchema.ENCOUNTER 
   JOIN @resultsDatabaseSchema.N3C_COHORT ON ENCOUNTER.PATID = N3C_COHORT.PATID 
   AND ADMIT_DATE >= TO_DATE(TO_CHAR(2018,'0000')||'-'||TO_CHAR(01,'00')||'-'||TO_CHAR(01,'00'), 'YYYY-MM-DD')
UNION 
SELECT 'CONDITION'  TABLE_NAME,
   count(*)  as ROW_COUNT
...
UNION 
SELECT 'DEATH'  TABLE_NAME,
   count(*) as ROW_COUNT FROM @cdmDatabaseSchema.DEATH 
   JOIN @resultsDatabaseSchema.N3C_COHORT ON DEATH.PATID = N3C_COHORT.PATID
UNION 
SELECT 'DEATH_CAUSE'  TABLE_NAME,
   count(*) as ROW_COUNT FROM @cdmDatabaseSchema.DEATH_CAUSE 
   JOIN @resultsDatabaseSchema.N3C_COHORT ON DEATH_CAUSE.PATID = N3C_COHORT.PATID
UNION 
SELECT 'DIAGNOSIS'  TABLE_NAME,
   count(*) as ROW_COUNT FROM @cdmDatabaseSchema.DIAGNOSIS 
   JOIN @resultsDatabaseSchema.N3C_COHORT ON DIAGNOSIS.PATID = N3C_COHORT.PATID 
   AND (DX_DATE >= TO_DATE(TO_CHAR(2018,'0000')||'-'||TO_CHAR(01,'00')||'-'||TO_CHAR(01,'00'), 'YYYY-MM-DD') OR ADMIT_DATE >= TO_DATE(TO_CHAR(2018,'0000')||'-'||TO_CHAR(01,'00')||'-'||TO_CHAR(01,'00'), 'YYYY-MM-DD'))
UNION
SELECT 'DISPENSING'  TABLE_NAME,
   count(*) as ROW_COUNT FROM @cdmDatabaseSchema.DISPENSING 
   JOIN @resultsDatabaseSchema.N3C_COHORT ON DISPENSING.PATID = N3C_COHORT.PATID 
   AND DISPENSE_DATE >= TO_DATE(TO_CHAR(2018,'0000')||'-'||TO_CHAR(01,'00')||'-'||TO_CHAR(01,'00'), 'YYYY-MM-DD')
UNION
SELECT 'IMMUNIZATION'  TABLE_NAME,
   count(*) as ROW_COUNT FROM @cdmDatabaseSchema.IMMUNIZATION 
   JOIN @resultsDatabaseSchema.N3C_COHORT ON IMMUNIZATION.PATID = N3C_COHORT.PATID
UNION
SELECT 'LAB_RESULT_CM'  TABLE_NAME,
   count(*) as ROW_COUNT FROM @cdmDatabaseSchema.LAB_RESULT_CM 
   JOIN @resultsDatabaseSchema.N3C_COHORT ON LAB_RESULT_CM.PATID = N3C_COHORT.PATID 
   AND (LAB_ORDER_DATE >= TO_DATE(TO_CHAR(2018,'0000')||'-'||TO_CHAR(01,'00')||'-'||TO_CHAR(01,'00'), 'YYYY-MM-DD') OR RESULT_DATE >= TO_DATE(TO_CHAR(2018,'0000')||'-'||TO_CHAR(01,'00')||'-'||TO_CHAR(01,'00'), 'YYYY-MM-DD'))
UNION
SELECT 'LDS_ADDRESS_HISTORY'  TABLE_NAME,
   count(*) as ROW_COUNT FROM @cdmDatabaseSchema.LDS_ADDRESS_HISTORY 
   JOIN @resultsDatabaseSchema.N3C_COHORT ON LDS_ADDRESS_HISTORY.PATID = N3C_COHORT.PATID
	AND (ADDRESS_PERIOD_END is null OR ADDRESS_PERIOD_END >= TO_DATE(TO_CHAR(2018,'0000')||'-'||TO_CHAR(01,'00')||'-'||TO_CHAR(01,'00'), 'YYYY-MM-DD'))
UNION
SELECT 'MED_ADMIN'  TABLE_NAME,
   count(*) as ROW_COUNT FROM @cdmDatabaseSchema.MED_ADMIN 
   JOIN @resultsDatabaseSchema.N3C_COHORT ON MED_ADMIN.PATID = N3C_COHORT.PATID 
   AND MEDADMIN_START_DATE >= TO_DATE(TO_CHAR(2018,'0000')||'-'||TO_CHAR(01,'00')||'-'||TO_CHAR(01,'00'), 'YYYY-MM-DD')
UNION 
SELECT 'OBS_CLIN'  TABLE_NAME,
   count(*) as ROW_COUNT FROM @cdmDatabaseSchema.OBS_CLIN 
   JOIN @resultsDatabaseSchema.N3C_COHORT ON OBS_CLIN.PATID = N3C_COHORT.PATID 
   AND OBSCLIN_START_DATE >= TO_DATE(TO_CHAR(2018,'0000')||'-'||TO_CHAR(01,'00')||'-'||TO_CHAR(01,'00'), 'YYYY-MM-DD')
UNION
SELECT 'OBS_GEN'  TABLE_NAME,
   count(*) as ROW_COUNT FROM @cdmDatabaseSchema.OBS_GEN 
   JOIN @resultsDatabaseSchema.N3C_COHORT ON OBS_GEN.PATID = N3C_COHORT.PATID 
   and OBSGEN_START_DATE >= TO_DATE(TO_CHAR(2018,'0000')||'-'||TO_CHAR(01,'00')||'-'||TO_CHAR(01,'00'), 'YYYY-MM-DD')
UNION
SELECT 'PRESCRIBING'  TABLE_NAME,
   count(*) as ROW_COUNT FROM @cdmDatabaseSchema.PRESCRIBING 
   JOIN @resultsDatabaseSchema.N3C_COHORT ON PRESCRIBING.PATID = N3C_COHORT.PATID 
   AND RX_START_DATE >= TO_DATE(TO_CHAR(2018,'0000')||'-'||TO_CHAR(01,'00')||'-'||TO_CHAR(01,'00'), 'YYYY-MM-DD')
UNION
SELECT 'PRO_CM'  TABLE_NAME,
   count(*) as ROW_COUNT FROM @cdmDatabaseSchema.PRO_CM 
   JOIN @resultsDatabaseSchema.N3C_COHORT ON PRO_CM.PATID = N3C_COHORT.PATID 
   AND PRO_DATE >= TO_DATE(TO_CHAR(2018,'0000')||'-'||TO_CHAR(01,'00')||'-'||TO_CHAR(01,'00'), 'YYYY-MM-DD')
UNION
SELECT 'PROCEDURES'  TABLE_NAME,
   count(*) as ROW_COUNT FROM @cdmDatabaseSchema.PROCEDURES 
   JOIN @resultsDatabaseSchema.N3C_COHORT ON PROCEDURES.PATID = N3C_COHORT.PATID 
   AND PX_DATE >= TO_DATE(TO_CHAR(2018,'0000')||'-'||TO_CHAR(01,'00')||'-'||TO_CHAR(01,'00'), 'YYYY-MM-DD')
UNION
SELECT 'PROVIDER'  TABLE_NAME,
   count(*) as ROW_COUNT FROM @cdmDatabaseSchema.PROVIDER
UNION 
SELECT 'VITAL'  TABLE_NAME,
count(*) as ROW_COUNT
FROM @cdmDatabaseSchema.VITAL 
JOIN @resultsDatabaseSchema.N3C_COHORT ON VITAL.PATID = N3C_COHORT.PATID 
AND MEASURE_DATE >= TO_DATE(TO_CHAR(2018,'0000')||'-'||TO_CHAR(01,'00')||'-'||TO_CHAR(01,'00'), 'YYYY-MM-DD')
;
